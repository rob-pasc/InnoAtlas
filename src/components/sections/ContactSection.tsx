...
const orgs: OrgContact[] = [
  {
    id: 'fhv',
    name: 'FHV Forschungszentrum',
    subtitle: 'Vorarlberg University of Applied Sciences',
    email: 'info@fhv.at',
    phone: '+43 5572 792',
    address: ['CAMPUS V, Hochschulstraße 1', '6850 Dornbirn', 'Österreich'],
  },
  {
    id: 'w4',
    name: 'W4',
    ...
    address: ['c/o Universität Konstanz', 'Postfach 207', '78457 Konstanz, Deutschland'],
  },
]

function OrgCard({ org }: { org: OrgContact }) {
  return (
        ...
        <div className="flex flex-col gap-4">
          {org.persons.map((p) => (
            <div key={p.name} className="flex flex-col gap-0.5">
              ...
              <span className="type-small text-fhv-black opacity-60">{p.role}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <section className="bg-fhv-white px-4 py-8 md:px-16 md:py-16">
      <div className="flex">
        ...
        <div className="flex-1 md:pl-5">
          <h2 className="type-hero text-fhv-black mb-5">Ansprechpersonen</h2>
